...
public class GetName implements IGetAPI, ISave {
    ...
    private Component NameEvent(ParameterList t){
        String c=Click.replace("[PlayerName]",t.getPlayer().getName());
        Component h= Color.colorInHex(Hover.replace("[PlayerName]",t.getPlayer().getName()));
        Component p=Color.colorInHex(format.getFirst()+t.getPlayer().getName()+format.getLast());
